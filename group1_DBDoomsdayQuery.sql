...
/* 
13. List all people along with the food inventory they are associated with.
*/
SELECT P.PeopleFirstName, P.PeopleLastName, F.FoodName, F.FoodQuantity
FROM People P
JOIN PeopleLodging PL ON P.PeopleID = PL.PeopleID
JOIN Lodging L ON PL.LodgingID = L.LodgingID
JOIN LocationInventory LI ON L.LocationID = LI.LocationID
JOIN Inventory I ON LI.InventoryID = I.InventoryID
JOIN InventoryFood Inv_Food ON I.InventoryID = Inv_Food.InventoryID
JOIN Food F ON Inv_Food.FoodID = F.FoodID;
